...
class GPT(nn.Module):
    ...
    def forward(
        ...
        ), f"Cannot forward sequence of length {T} through a transformer with block size {self.config.block_size}"
...
for step in range(50):
    ...
    print(
        f"step: {step:4d} | loss: {loss.item():.6f} | lr: {lr:.4f} | norm: {norm:.4f} | dt: {dt:.2f}ms | tok/sec: {tokens_per_sec:.2f}"
    )
